...
private:
    ...
    /*!
     * Returns the child node of 'node' at position 'idx'.
     * If not such child node exists and 'createIfMissing' is true, a new child node is created at position 'idx'.
     *
     ...
     * @param createIfMissing Creates a new child node at position 'idx' if one doesn't already exist.
     * @result Indicates the success of the operation (it's true IFF 'node' has a child at position 'idx').
     */
    bool getChildNode(Node *node, int idx, bool createIfMissing);
